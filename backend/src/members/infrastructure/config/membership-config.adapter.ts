...
export class PrismaMembershipConfigAdapter implements MembershipConfigAdapter {
  ...
  async getAllowReRegistration(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_ALLOW_RE_REGISTRATION', true);
  }

  async getRegistrationNumberFormat(): Promise<string> {
    return this.configService.getSystemSetting('MEMBERSHIP_REG_NUMBER_FORMAT', 'SEQUENTIAL') || 'SEQUENTIAL';
  }

  async getRegistrationNumberPrefix(): Promise<string> {
    return (this.configService.getSystemSetting('MEMBERSHIP_REG_NUMBER_PREFIX', '') ?? '').trim();
  }

  async getRegistrationNumberStart(): Promise<number> {
    return this.configService.getSystemSettingNumber('MEMBERSHIP_REG_NUMBER_START', 1);
  }

  async getAutoGenerateRegistrationNumber(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_AUTO_GENERATE_REG_NUMBER', false);
  }

  async getDefaultStatus(): Promise<string> {
    return this.configService.getSystemSetting('MEMBERSHIP_DEFAULT_STATUS', 'PENDING') || 'PENDING';
  }

  async getAutoApprove(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_AUTO_APPROVE', false);
  }

  async getRequireBoardDecision(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_REQUIRE_BOARD_DECISION', false);
  }
  ...
  async getRequireEmail(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_REQUIRE_EMAIL', false);
  }

  async getRequireInstitutionRegNo(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_REQUIRE_INSTITUTION_REG_NO', false);
  }

  async getRequireWorkUnit(): Promise<boolean> {
    return this.configService.getSystemSettingBoolean('MEMBERSHIP_REQUIRE_WORK_UNIT', false);
  }
}
