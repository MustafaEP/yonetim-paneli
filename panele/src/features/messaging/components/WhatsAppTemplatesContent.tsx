...
const WhatsAppTemplatesContent: React.FC = () => {
  ...
  const usedTriggerEvents = useMemo(() => {
    ...
    return new Set(
      templates.filter((t) => t.isActive && t.triggerEvent).map((t) => t.triggerEvent!),
    );
  }, [templates]);
  ...
  const insertVariable = (variable: string) => {
    setForm({ ...form, content: `${form.content} ${variable}`.trim() });
  };
                  ...
                  <TableRow key={template.id} hover>
                    ...
                    <TableCell>
                      {template.triggerEvent ? (
                        <Chip
                          label={TRIGGER_EVENTS.find((e) => e.value === template.triggerEvent)?.label || template.triggerEvent}
                          size="small"
                          color="primary"
                          variant="outlined"
                          sx={{ fontSize: '0.7rem' }}
                    ...
                    <TableCell>
                      <Chip
                        ...
                        sx={{
                          fontSize: '0.7rem',
                          fontWeight: 600,
                          backgroundColor: template.isActive ? alpha('#4caf50', 0.1) : alpha('#f44336', 0.1),
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              helperText={
                TRIGGER_EVENTS.find((e) => e.value === form.triggerEvent)?.description || 'Otomatik gönderim için bir olay seçin'
              }
            >
              {TRIGGER_EVENTS.map((event) => {
                const isUsed =
                  ...
                  event.value !== (editingId ? templates?.find((t) => t.id === editingId)?.triggerEvent : undefined);
                return (
        ...
        <DialogActions>
          ...
          <Button
            onClick={handleSave}
            variant="contained"
            disabled={!form.name || !form.slug || !form.content || createTemplate.isPending || updateTemplate.isPending}
...
export default WhatsAppTemplatesContent;
